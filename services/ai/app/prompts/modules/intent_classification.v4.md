...
A single JSON object with exactly these keys, and no others. Two worked
examples — copy the **shape**, never the values.

For *"is my business partner trustworthy"*:
...
```
{
  ...
  "entities": { "timeframe": "", "person": "my business partner", "topic": "trust" }
}
```

For *"will my daughter's marriage happen next year"*:
...
`entities`, not at the top level.

Use `""` for an entity you did not find — an empty string, and **never `null`**.
`null` is only ever the value of `secondary`.

Give a real `confidence` every time, from the scale in rule 2. It is the one
field with no safe default: omit it or copy one and the label is discarded.
